...
def test_analytics_requires_pro(owner_session, base_url):
...
def test_analytics_with_period_param(owner_session, base_url):
    r = owner_session.get(f"{base_url}/api/analytics", params={"period": "7d"})
    assert r.status_code == 403


def test_analytics_with_custom_dates(owner_session, base_url):
    r = owner_session.get(
